...
pub enum RunnerError {
    #[error("failed to spawn summary executable")]
    Spawn(#[source] io::Error),
    #[error("summary process I/O failed")]
    Io(#[source] io::Error),
...
pub fn run_bounded(config: &RunnerConfig, input: Vec<u8>) -> Result<Vec<u8>, RunnerError> {
    ...
    let status = loop {
        ...
        if started.elapsed() >= config.timeout {
            ...
            let (_stdin, _stdout, _stderr) =
                join_workers(stdin_worker, stdout_worker, stderr_worker)?;
            return Err(RunnerError::Timeout(config.timeout));
        }
...
fn join_workers(
    ...
    stdout: ReaderWorker,
    stderr: ReaderWorker,
) -> Result<(io::Result<()>, BoundedRead, BoundedRead), RunnerError> {
    let stdin = stdin.join().map_err(|_| RunnerError::WorkerPanic)?;
    let stdout = stdout
        .join()
...
fn validate_exit(status: ExitStatus, stderr_bytes: usize) -> Result<(), RunnerError> {
    if status.success() {
        Ok(())
    } else {
