# Munshi design and product specification

This is the full product specification and design record for Munshi, moved out of the README so
the README can stay a short introduction. It reads best top to bottom, but each section stands on
its own. For task-oriented guides, start with [getting-started.md](getting-started.md) instead.
...
above; see [ADR 0007](adr/0007-madari-status-and-actions-over-cli-json-only.md). See also
...
- Building a web dashboard. (A local read-only [backlog dashboard](dashboard.md) has since
  been adopted as an optional addon in issue #56, and a [desktop addon](gui.md) in ADR 0014;
  both consume the CLI `--json` contracts only, and Munshi itself still has no UI code path.)
...
[automatic session archival](automatic-archive.md).
...
[automatic session archival](automatic-archive.md#claude-code-hooks)), hook payloads are
version-pinned at Claude Code 2.1.205
([phase-0 findings](phase-0-claude-code-findings.md)), and recovery sweeps
...
[harness source adapters](harness-adapters.md). Synthetic/sanitized conformance fixtures live
...
disablement. See [`docs/automatic-archive.md`](automatic-archive.md#project-policy-and-cost-budgets).
...
See [ADR 0004](adr/0004-use-rebuildable-sqlite-operational-state.md).
...
[`docs/phase-0-probe.md`](phase-0-probe.md). Live results belong in the explicitly unobserved
[`docs/phase-0-findings.md`](phase-0-findings.md) matrix.
